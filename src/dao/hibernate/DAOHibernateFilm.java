...
public class DAOHibernateFilm extends DAOHibernate implements DAOFilm{
	...
	public ArrayList<Film> loadFilmValide() throws Exception {
		// TODO Auto-generated method stub
		Session	session = connect();
		ArrayList<Film> set=(ArrayList<Film>) session.createQuery("FROM Film WHERE isValidateFilm=-1 order by titre").list();	
		close(session);	
		return set;
	}
	@SuppressWarnings("unchecked")
	@Override
	public ArrayList<Film> loadFilmValide(String key) throws Exception {
		// TODO Auto-generated method stub
		Session	session = connect();
		ArrayList<Film> liste=(ArrayList<Film>) session.createQuery(
				"FROM Film as film WHERE isValidateFilm=-1 AND film.titre LIKE '%"+key+"%' order by titre").list();	
